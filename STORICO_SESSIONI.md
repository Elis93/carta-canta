...
# Handoff 13 luglio – 2 agosto 2026 — spostati da CLAUDE.md il 6 agosto

> Terzo consolidamento (i precedenti: 14 giugno e 15 luglio). CLAUDE.md era arrivato a
> 2.100 righe e viene riletto per intero all'inizio di ogni sessione: tenerci dentro tre
> settimane di cronaca costava a ogni avvio senza servire a nessuno.
> Qui sotto le voci dal 2 agosto a ritroso, nell'ordine in cui stavano (dalla più recente).
> **Le regole permanenti che erano annegate in questi handoff sono state estratte** e vivono
> ora in CLAUDE.md §B.2 ("Regole imparate sul campo"): qui restano solo per il contesto.
...
- **lib/actions/marketplace.ts**: `cleanProfile` + show_phone/public_email; `publicEmailError` (forma email, messaggio in italiano); `upsertProfileTollerante` pre-064 (42703/PGRST204 → retry senza contatti) usato dal save (il publish delega al save → copre entrambi).
- **Form profilo**: sezione "Contatti mostrati ai clienti" — 2 checkbox ("Il telefono — bottone Chiama" / "Un'email — meglio se dedicata, non quella di accesso") + campo email visibile solo a interruttore acceso (spento → si salva vuoto). Page con select tollerante.
- **Profilo pubblico /professionisti/[id]**: select tollerante pre-064; bottoni "Chiama" (tel:) e "Scrivi un'email" (mailto:) nell'intestazione SOLO se attivati.
- tsc+build+409/409 verdi. ⏭️ Collaudo Eli: modulo richiesta end-to-end (checklist inviata in chat) + i due interruttori post-064.

### ✅ 2 ago (27) — HOME: via i bordi oro dalle card + grigio scadenza più chiaro (richieste Eli)
- Rimosso il `borderLeft` oro `#e5d3a1` da TUTTE le card della Home (agenda, In scadenza, 2 KPI, attività, quota banner Free escluso: quello è un banner Pro con accento `#c9a44c` voluto). Contenuto della card In scadenza: grigio `#55534b` → `#6f6d64` (un filo più chiaro).
- ⏭️ VETRINA/CONTATTI: scoperto che il TELEFONO del profilo marketplace NON è mai mostrato al pubblico (il dettaglio /professionisti/[id] seleziona solo nome/mestiere/città/bio — il campo phone del form profilo oggi non compare da nessuna parte); il contatto passa SOLO dal modulo richiesta. Proposta a Eli (in attesa di scelta): 2 interruttori opt-in nel profilo ("Mostra il telefono" + "Mostra un'email in vetrina" con campo email PUBBLICA separata da quella di login), default spenti, modulo sempre presente; migration 064 con ⚠️ GRANT per colonna su marketplace_profiles (lezione 045×055).
- tsc+build+409/409 verdi.

### ✅ 2 ago (26) — CARD "IN SCADENZA" unificata in Home + BUG LOGO (React 19 form reset) + Salva fiscale rialzato
- **Card unica "In scadenza"** (mockup approvato con 2 modifiche di Eli: SOLLECITA anche per la fattura, tasti compressi): nuovo `ScadenzeHomeCard` — blocco PREVENTIVO + blocco FATTURA DA INCASSARE (entrambi con Sollecita per mail/WhatsApp/chiama; testo WhatsApp diverso per tipo; `sendReminderAction(id, kind)` supporta già le fatture incl. 'expired'), tasti compatti "Preventivi (N) →" / "Fatture (N) →" (badge = entro 7 giorni, stessa semantica del vecchio badge di Altro) → **voce "Scadenze" RIMOSSA da Altro** (con query e badge; riga Bilancio ora `last`; la pagina hub /scadenze resta raggiungibile ma non linkata). Dashboard: 2 query nuove nel Promise.all (fattura più urgente sent/viewed/expired + count 7g); `MobileScadenzaCard` ELIMINATA. Harness Chromium 390px: replica fedele del mockup, 0 overflow.
- **[BUG] Logo "Nessun file selezionato"** (foto Eli): il campo file vive nella form del "Salva" dati e **React 19 azzera la form dopo OGNI submit** → toccando Salva il file spariva dall'input e il successivo "Carica" partiva a vuoto. Fix: il File vive nello STATO (`logoFile`, sopravvive a qualsiasi submit), upload via `startTransition(() => logoAction(fd))` con FormData costruita dallo stato; l'input non ha più `name` (Salva non trascina i byte); **"Carica il logo" ora è un BOTTONE vero** (navy, 44px) + Annulla; accept allineato alla allowlist server (via svg, lezione pentest 20 lug).
- **"Salva impostazioni fiscali" sopra il "+"**: `paddingBottom: 44` sul contenitore del bottone (finiva sotto il FAB della bottom-nav a fondo pagina).
- tsc+build+409/409 verdi.
...
- Verifica visiva Chromium 390px (chip fermo + in modifica sul fondo nuovo). tsc+build+409/409 verdi.

### ✅ 2 ago (19) — RI-REVIEW della Fase 2 (richiesta Eli "ricontrolla"): 2 fix + 8 test con mock sulle action
Rilettura critica dell'intero diff Fase 2 per classi di bug note. **Verificati PULITI**: superfici pubbliche (/p seleziona colonne esplicite — né unit_cost né supplier_list_id nel payload del cliente; grep B.2 su lib/pdf, app/p, app/r, email = solo commenti "margine di pagina"); tutti e 5 gli insert di document_items passano dal helper tollerante (incluso duplica); nessun doppio conteggio quota AI (/api/ai/extract registra solo il cap elaborazioni, l'import si conta al salvataggio, identico al catalogo); restore-version pre-esistente non tocca le colonne nuove; convert SQL a colonne esplicite non rompe. **Fixati**: ① `ListinoDetail.saveInfo` senza la validazione range del ricarico (un 9999 arrivava al server → messaggio Zod in inglese) → stessa validazione 0-500 di NuovoListinoForm; ② name/description assenti nel FormData → null → Zod "Invalid input: expected string" in inglese → coalescing a '' così scatta il `min(1)` col messaggio italiano (beccato da un test nuovo). **+8 test** `tests/unit/fornitori/actions.test.ts` (mock catena Supabase per-tabella): Pro gate senza scritture · uuid invalido senza toccare la quota · quota esaurita = zero insert/update e uso NON registrato · pre-063 → messaggio migration · primo import (insert + recordAiImportUse col conteggio) · RINNOVO (match per codice, update costo, stats +10%, nuova scadenza, inserita solo la voce nuova) · 2 su createSupplierListAction. **409/409** verdi · tsc+build ok · scan spazi puliti.

### ✅ 2 ago (18) — FASE 2 LISTINO FORNITORE IMPLEMENTATA: listini con scadenza + import/rinnovo AI (✅ migration 063 APPLICATA da Eli)
Eli: "procedi con la fase 2". Implementata per intero (mappa aggiornata in `PROGETTO_LISTINO_FORNITORE.md` §Fase 2):
- **⚠️ migration 063** (`063_listini_fornitori.sql`, VALIDATA su PG16 reale — idempotente; delete listino → voci CASCADE, `document_items.supplier_list_id` a NULL e costo congelato intatto): `supplier_lists` (nome, markup_pct, valid_until) + `supplier_list_items` (code/description/unit/unit_cost, CHECK ≥0) + `document_items.supplier_list_id` FK SET NULL. RLS `my_workspace_ids` su entrambe.
- **`lib/fornitori/listino.ts`** (puro, **9 test**): `prezzoProposto` (costo+ricarico; senza ricarico → null = "da prezzare", MAI un prezzo inventato) · `matchRinnovo` (per codice poi descrizione normalizzata; stats "N rincarate, media +X%") · `riepilogoRinnovo` · `giorniAllaScadenza`. **`lib/actions/fornitori.ts`**: CRUD Pro-gated tollerante pre-063 + `importSupplierItemsAction` (import AI E rinnovo nello stesso punto: listino con voci → abbina/aggiorna/aggiunge; stessa quota AI dell'import catalogo, consumata al salvataggio).
- **UI**: /catalogo → **"Catalogo e listini"** a 2 linguette (Free → upsell Pro); `NuovoListinoForm` (ricarico default 25%); dettaglio **`/catalogo/fornitori/[id]`** (`ListinoDetail`: info inline, import/RINNOVA con foto/PDF via `/api/ai/extract`, voci "costo → vendi a", CRUD). **CatalogPicker a 2 linguette** (plan letto client-side): la voce dal listino entra con costo + prezzo proposto + supplier_list_id. **Aggancio scadenza (pilastro D)**: validità controllata + banner ambra nel form quando un listino usato scade prima della validità, bottone "[Allinea: valido N giorni]"; listino scaduto → invito a ricontrollare i prezzi. Voce Altro rinominata.
- **Filo `supplier_list_id`**: VoceSchema + `sanitizeSupplierListId` + 5 insert mapping + re-edit + duplica; `insertDocumentItemsTollerante` a CASCATA (42703/23503 → senza listino → senza costo). **Foto-AI**: il match catalogo porta anche `unit_cost` (limite F1 chiuso, select tollerante pre-062).
- 🔒 B.2 verificata: grep pulito su lib/pdf, app/p, app/r, email. Copy: FAQ in /aiuto + voce /novita.
- tsc+build+**401/401** verdi · scan spazi puliti · verifica visiva Chromium 390px su ListinoDetail reale (0 overflow, 2,50+25% = 3,13 ✓). ⏭️ FASE 3 (campanella listino_scaduto, manodopera nel margine, avviso duplica) da fare su richiesta.
...
- **Verifica end-to-end con Chromium sul componente REALE** (harness esbuild: stub di supabase client e @simplewebauthn, route passkey mockate, pagina servita su origine http vera — su about:blank localStorage non funziona e il lock non parte): sequenza lock → tap impronta → hidden → 800ms → sblocco → visible in ritardo. Pre-fix: RI-BLOCCA ✗ · post-fix: nessun ri-blocco ✓ · controprova: vera uscita (hidden da sbloccato) ri-blocca come previsto ✓.
...
- tsc+build+392/392 verdi · scan spazi pulito (solo il falso positivo noto "furgone").

### ✅ 2 ago (13) — NUMERO in Georgia + modificabile dal chip · RAPPORTINO scaricabile (cliente E artigiano)
- **Chip numero elegante e cliccabile** (Eli: "carattere più elegante e se ci clicco voglio poter modificare le cifre, idem fattura"): font `Georgia, 'Times New Roman', serif` (14.5px, # oro) sui chip di PreventivoForm e FatturaForm. Il chip si TOCCA in creazione preventivo (prefill col nextDocNumber), modifica preventivo e modifica fattura (rimosso il vecchio campo dalla Card 1, header 'Fattura'→'Cliente'). **Rifinitura richiesta da Eli ("si modifichi dove è e basta"): modifica IN-PLACE** — niente pannello: al tocco il chip stesso diventa un input (stesso stile tratteggiato oro, autoFocus, Enter con preventDefault per non inviare il form in create, blur → validazione e ritorno a chip; errore in rosso sotto la riga + bordo chip rosso). Verificata con replica Chromium a 390px (0 overflow; su mobile l'input sale a 16px per la regola anti-zoom → width 92 basta). ⚠️ ANTI-COLLISIONE: in create il numero viaggia in un hidden che manda '' se il valore è rimasto IDENTICO al nextDocNumber proposto → lo assegna la sequenza (un "manuale" uguale al prossimo non incrementa la sequenza e farebbe collidere il documento successivo — verificato su createDocumentAction:459-471). La CREAZIONE fattura (FatturaForm) resta chip informativo (B.3: il server ignora l'override).
- **Rapportino come DOCUMENTO** (Eli: "visualizzare come html o scaricare, sia per il cliente che l'artigiano"): nuovo `lib/pdf/rapportino.ts` (`buildRapportinoHtml` — escape HTML su ogni campo, header Georgia col bordo oro, sezioni Lavori eseguiti/ore/foto 2-col page-break-inside avoid/firma verde o avviso ambra, footer Carta Canta; 🔒 B.2 zero costi/margini) + 2 route con `preparePrintHtml` (B.8, `?preview=1` senza stampa): **`/api/r/[token]/pdf`** pubblica (UUID regex come sign, rate-limit `pdf-r:` 20/min, cascate 42703 pre-052/053, foto SOLO visible_to_client) e **`/api/lavori/[id]/rapportino-pdf`** autenticata (workspace owner/membro). Bottoni: "Scarica il rapportino (PDF)" su `/r/[token]` (sotto la firma) e nella RapportinoCard (quando il link esiste, firmato o no). Verifica visiva Chromium sull'HTML reale (firmato + non firmato).
- tsc+build+392/392 verdi · scan spazi pulito (build+sorgente). Da collaudare da Eli: chip in creazione/modifica, download rapportino dal telefono.
...
- **/aiuto, 2 FAQ nuove**: "I miei dati restano miei?" (tutti gli export + Account e dati + "nessun vincolo, disdici quando vuoi") e "L'assistenza è solo per chi paga?" ("per tutti, anche Free, risponde una persona"). Aggiornata la FAQ foto al nuovo percorso (Opzioni → Dalle foto). /abbonamento diceva già "Nessun contratto. Cancella quando vuoi."
- **Restano GATED su avvocato (B.0)**: "promessa prezzi" su landing (claim AGCM) e solleciti automatici fatture scadute (email a clienti finali, opt-in) — annotati in PROGETTO_LISTINO_FORNITORE.md §"Altre due mosse"; da inserire nel dossier unico avvocato alla prossima rigenerazione (il sorgente del dossier non è nel repo, per regola).
- tsc+build+392/392 verdi · scan spazi pulito.
...
### ✅ 2 ago (3) — LOTTO FEEDBACK ELI (4 fix): lavori in Home, tendina "Opzioni" nel form, punto ⓘ SdI, tab respirano in Testo grande
- **Lavori nelle Attività recenti (Home)**: il feed ora unisce documenti E lavori (query `lavori` tollerante pre-049 con `.then(ok, ko)`, finestra 6 mesi, limit 5), ordinati per updated_at; riga lavoro con badge di stato (LAVORO_STATUS_META), "Lavoro creato/aggiornato il X", href /lavori/[id]. Mobile + desktop.
- **Tendina "Opzioni" nella card Voci** (preventivi E fatture in edit — la creazione fattura non aveva AI): "Importa con AI", "Compila dalle note (AI)", "Proponi le voci dalle foto (AI)" e il toggle "Proponi più opzioni" vivono in un pannello CHIUSO di default (chip "Opzioni ▾" nell'header della card) — li vede solo chi li usa, il form resta pulito (era la confusione segnalata da Eli). ⚠️ Le linguette Base/Premium restano SEMPRE visibili quando le proposte sono attive (fuori dal pannello). Il tutorial regge: markTour ignora gli elementi non presenti (ai-foto dentro la tendina chiusa).
- **Punto ⓘ sulla card SdI**: bottone info accanto al titolo → pannello in parole semplici (fattura elettronica obbligatoria, il PDF è copia di cortesia, cos'è lo SdI, esiti Consegnata/Scartata coi 5 giorni, alternativa cassetto/commercialista, "una volta sola").
- **Testo grande, "Rifiutati" a filo del bordo**: il padding destro di un contenitore che scorre viene mangiato a fine scroll (quirk CSS) → margini sui figli (`:first-child`/`:last-child` in `.cc-tabs.cc-filter-scroll`), misurato con Chromium a 360px+zoom 1.15: 14px d'aria a fine scroll. ⚠️ REGOLA: in contenitori overflow-x lo spazio finale si dà coi MARGINI dei figli, non col padding del contenitore.
- **"Sfridi"**: confermato corretto (termine tecnico standard per gli scarti di taglio) — copy invariata.
- tsc+build+392/392 verdi · scan spazi puliti. ⏭️ Eli deve ancora collaudare le RECENSIONI (si sbloccano a fattura saldata per intero).

### ✅ 2 ago (2) — FASE 1 LISTINO FORNITORE IMPLEMENTATA: costo per voce + margine privato (⚠️ migration 062 DA APPLICARE)
Decisioni congelate da Eli ("ok a tutto"): margine GRATIS per tutti · listini fornitori = Pro (F2) · "Catalogo e listini" una pagina con 2 linguette (F2) · margine a tendina. Implementata la F1:
- **⚠️ migration 062** (`062_costi_margine.sql`, VALIDATA su PG16 reale — idempotente, convert copia il costo): `unit_cost DECIMAL(10,2) NULL` su catalog_items E document_items (il costo si CONGELA sulla voce del documento) + `convert_preventivo_to_fattura` aggiornata.
- **`lib/margine/calcolo.ts`** (puro, 11 test): sconto VOCE dentro il margine della voce; sconto DOCUMENTO sottratto una volta sola dal totale (mai spalmato — l'errore di Jobber); % di documento SOLO se tutte le voci hanno un costo (alla Quotient); sotto-costo negativo mai mascherato.
- **UI**: campo "Costo (quanto la paghi) — facoltativo" nel form catalogo (+ "costo X €" nella riga mobile); in VociTable link per voce "🔒 Costo e margine (solo per te)" → campo + riga privata "ricarico X% · margine +Y €" (rossa se sotto costo); **MargineBox** nel PreventivoForm sopra il riepilogo (tendina chiusa di default, pattern Cronologia). Tutto invisibile a chi non usa i costi. CatalogPicker porta il costo nella voce.
- **Server**: VoceSchema + unit_cost; 5 mapping insert + `insertDocumentItemsTollerante` (pre-062 ritenta senza colonna: nessun salvataggio si rompe prima della migration); azioni catalogo con retry tollerante.
- **🔒 Test privacy della regola B.2** (`tests/unit/margine/privacy-pdf.test.ts`): i 4 preset PDF non contengono mai costo/"margine"/"ricarico" (valore sentinella). Verificato che /p/[token] seleziona colonne esplicite → il costo non entra nel payload RSC del cliente. ⚠️ REGOLA: ogni select nuova verso superfici pubbliche resta a colonne esplicite.
- Verifica visiva Chromium 390px (esbuild+stub): riga privata ✓, tendina chiusa/aperta ✓ (195 − 50 = +145 €). tsc+build+**392/392** verdi · scan spazi puliti. Limiti noti minori annotati nel progetto (restore-version senza costi; FatturaForm create senza box; foto-AI senza costo → F2).

### ✅ 2 ago — PROGETTO "Listino fornitore, costi e margine" APPROVATO (design chiuso, da implementare)
Da feedback diretto di un artigiano ("preparo il preventivo in base al margine sul fornitore; il listino dura 10 giorni"). Ricerca web (Tolteck/Quotient/Jobber/Obat/Fatture in Cloud) + mockup 5 schermate approvato da Eli. **Mappa completa in `PROGETTO_LISTINO_FORNITORE.md`**: 4 pilastri (due prezzi per voce · ricarico proposto · margine privato · scadenza listino agganciata alla validità del preventivo), gestione sconto senza contraddizioni (sconto voce → riga voce; sconto documento → SOLO riquadro totale, mai spalmato — l'errore di Jobber), 7 flussi (A-G), 3 fasi (F1 costo+margine · F2 listini fornitori con scadenza+import AI · F3 guardiano campanella). **🔒 NUOVA REGOLA PERMANENTE in B.2: costo/ricarico/margine MAI al cliente** (PDF, /p, /r, email, snapshot — filtro alla fonte + test). Domanda aperta a Eli: gating (proposta: campo costo per tutti, margine live = Pro). Decisioni collaterali di Eli: ✅ "promessa prezzi"+supporto umano (copy → avvocato) · ✅ solleciti automatici fatture scadute (opt-in, avvocato PRIMA, B.0) · ❌ riconciliazione bancaria non ora.
...
- **Richieste vetrina in CAMPANELLA**: nuovo tipo `richiesta` in lib/notifications.ts (marketplace_requests status='new', tollerante 043, key `mkreq:{id}`, href /richieste) + icona MessageSquare in NotificationList + toggle "Richieste dalla vetrina" (`inapp_richiesta`, default ON) in Impostazioni›Notifiche (schema Zod + mapping impostazioni/page).
- **Riepilogo al CLIENTE che invia la richiesta**: nuovo template `marketplace_richiesta_cliente.tsx` (transazionale avviata dal cliente; messaggio+comune nel corpo; "Carta Canta non è parte del rapporto") inviato best-effort SOLO se il recapito è un'email; conferma in-page aggiornata ("Ti abbiamo inviato un riepilogo via email" solo in quel caso — col telefono nessuna email possibile).
- tsc+build+377/377 verdi.

### ✅ 29 lug (5) — CHECKSUM P.IVA/CF anche in RUBRICA CLIENTI + pop-up guidato "Vicino a me" (richieste Eli)
- **ClientForm**: "P.IVA rilevata ✓" compariva anche con una cifra SBAGLIATA (controllava solo la forma). Ora: P.IVA a 11 cifre → checksum `isValidPivaFormat` (avviso ambra non bloccante — la P.IVA cliente errata è tra le prime cause di scarto SdI 00305); CF a 16 → formato con omocodia (o CF ente 11 cifre). **IBAN già coperto** (mod-97 bloccante in updateWorkspacePayment); codice destinatario/PEC già validati nel dialog SdI → nessun altro campo scoperto.
- **NearMeButton**: via la riga di testo "riattivala da Impostazioni…" → **pop-up guidato** ("Attiva la posizione") con 2 varianti: permesso negato e GPS spento (tendina impostazioni rapide), bottoni **Riprova** (rilancia getCurrentPosition) e "Cerca per comune". Il caso "permesso negato" ha **istruzioni diverse per contesto** (fix su segnalazione Eli "ho installato la app, non c'è il lucchetto"): app installata (display-mode standalone) → ⚠️ verificato sul telefono di Eli: il pannello Autorizzazioni delle PWA da Chrome mostra SOLO "Notifiche" — la Posizione vive nei permessi del SITO in Chrome (condivisi con l'app) → istruzione "apri Chrome su cartacanta.app → lucchetto → Autorizzazioni → Posizione → Consenti"; browser → lucchetto accanto all'indirizzo. ⚠️ LIMITE WEB spiegato a Eli: nessun sito può aprire le impostazioni del telefono né riproporre il prompt dopo un rifiuto — il prompt nativo compare da solo quando possibile; il pop-up copre il resto. Verificato con Chromium (geolocation mockata, entrambe le varianti a 390px). tsc+build+377/377 verdi.
...
- ✅ **ATTIVATA E COLLAUDATA IN PRODUZIONE (5 ago 2026)**: Eli ha attivato l'API "Company" su console.openapi.com (richiede verifica d'identità + firma dell'addendum "informazioni commerciali" ai sensi del T.U.L.P.S. — è un obbligo di legge per chi vende informazioni commerciali, non una richiesta del fornitore), generato il token di produzione con scope `GET /IT-start` e messo `OPENAPI_COMPANY_API_KEY` su Vercel + Redeploy. **Prezzo reale: 0,050 €/chiamata con le prime 30 al mese GRATIS** → in beta il costo previsto è zero (si interroga solo quando il VIES non conferma, una volta per artigiano che pubblica). Abbonamento NON preso: ha senso solo a volumi. **Collaudo end-to-end riuscito**: P.IVA vera non presente nel VIES → pubblicazione consentita con la dicitura **"Riscontro automatico sul Registro Imprese"** (quindi la strada NUOVA, non il ripiego VIES); P.IVA inventata → pubblicazione BLOCCATA. ⚠️ Aggiunto al dossier avvocato (in COSE_DA_FARE_ELI §2) e **dichiarato nell'informativa privacy** (Openapi tra i destinatari, §2-bis/§3/§5).
...
- **DEFERRED confermato**: l'acconto "migra" di mese quando si incassa il saldo (`paid_at` sovrascritto) — ~~serve la tabella storia incassi~~ ✅ RISOLTO 4 ago SENZA tabella: il Bilancio legge la storia dal `document_log` (`lib/bilancio/incassi.ts`).
