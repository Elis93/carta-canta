...
- **NearMeButton**: via la riga di testo "riattivala da Impostazioni…" → **pop-up guidato** ("Attiva la posizione") con 2 varianti: permesso negato (gesto esatto: lucchetto → Autorizzazioni → Posizione) e GPS spento (tendina impostazioni rapide), bottoni **Riprova** (rilancia getCurrentPosition) e "Cerca per comune". ⚠️ LIMITE WEB spiegato a Eli: nessun sito può aprire le impostazioni del telefono né riproporre il prompt dopo un rifiuto — il prompt nativo compare da solo quando possibile; il pop-up copre il resto. Verificato con Chromium (geolocation mockata, entrambe le varianti a 390px). tsc+build+377/377 verdi.
