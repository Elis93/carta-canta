...
- **Chip numero elegante e cliccabile** (Eli: "carattere più elegante e se ci clicco voglio poter modificare le cifre, idem fattura"): font `Georgia, 'Times New Roman', serif` (14.5px, # oro) sui chip di PreventivoForm e FatturaForm e sull'Input del pannello. Il chip ora si TOCCA anche in **creazione preventivo** (prefill col nextDocNumber → cifre editabili) e in **modifica fattura** (pannello con label "Numero fattura"; rimosso il vecchio campo dalla Card 1, header 'Fattura'→'Cliente'). ⚠️ ANTI-COLLISIONE: in create il numero viaggia in un hidden che manda '' se il valore è rimasto IDENTICO al nextDocNumber proposto → lo assegna la sequenza (un "manuale" uguale al prossimo non incrementa la sequenza e farebbe collidere il documento successivo — verificato su createDocumentAction:459-471). La CREAZIONE fattura (FatturaForm) resta chip informativo (B.3: il server ignora l'override).
