...
**001-074 TUTTE APPLICATE, per intero. ⚠️ La 075 è DA APPLICARE** («Archivio dei documenti + solleciti spenti»: `documents.archived_at` e `documents.reminders_off_at`, validata su PG16). Finché non è applicata l'app funziona come prima: liste, Home e campanella hanno la tolleranza pre-migration, e i comandi «Non ricordarmelo più» / «Archivia» rispondono *"manca un aggiornamento del database"* invece di rompere qualcosa.
La 074 («Posticipa il sollecito») è stata applicata da Eli l'8 ago, dopo la validazione su PG16.
