...
- **[MEDIA] "Sentry — UE" nell'informativa era un'affermazione NON verificabile**, e il codice suggerisce il contrario (`next.config.ts` allarga la CSP proprio per coprire l'ingest `*.us.sentry.io`; la sede di Sentry è USA). Regola B.0: a parità di alternative la più difendibile → **Sentry dichiarato USA** (coperto da DPF in §6; dichiarare USA quando fosse EU è innocuo, il contrario no) + verifica di 2 minuti per Eli in `COSE_DA_FARE_ELI.md` §2 (guardare la regione dell'org nel pannello Sentry: se EU, si sposta).
