...
> **Ultima sessione: 11 agosto 2026** (pilota automatico SdI con data fiscale alla conferma — migration 080 —, blocchi SdI in Home, i 10 scarti spiegati).
...
### ✅ 11 ago (7) — 🚁 PILOTA AUTOMATICO SdI + data fiscale alla CONFERMA (⚠️ migration 080 DA APPLICARE) + Home SdI + i 10 scarti spiegati
...
**001-079 applicate (la 079 l'ha applicata Eli il 10 ago). ⚠️ 080 DA APPLICARE.**
**080** («Data di conferma + pilota automatico SdI»: `documents.doc_date` — la data FISCALE, nasce alla conferma della bozza, backfill dei non-bozza con created_at in Europe/Rome — · `workspaces.sdi_auto_enabled` default true · `documents.sdi_auto_at` + indice parziale per il cron orario `/api/cron/sdi-auto`). VALIDATA su PG16 (fuso a cavallo di mezzanotte, bozze a NULL, idempotente, valori scritti intatti al rilancio). ⚠️ Applicabile **prima o dopo il deploy**: tutto il codice è tollerante pre-080; senza migration la data resta created_at e il pilota semplicemente non parte.
...
**Verifica prima di ogni rilascio:** `npx tsc --noEmit` · `npm run build` · `npm test` (646) · `npm run build && npm run smoke:public` (20 check) · `npm run security:check` (sito vero).
