...
### ⏳ 10 ago (3) — REVISIONE A COPPIE delle funzioni della settimana (richiesta Eli) — IN CORSO, fatte 2 su 22
Eli: *"crea una lista di tutte le funzioni aggiunte nell'ultima settimana e poi le ricontrolliamo due a due… non voglio che abbiamo dimenticato di aggiornare le cose collegate, tipo le FAQ"*. Lista di **22 funzioni** (3→10 ago) mandata in chat; si procede a coppie. ⚠️ **Se la sessione riparte da qui: chiedere a Eli se proseguire con la coppia successiva** (la 3-4: import listino da PDF · richieste dalla vetrina).
...
### Stato migration (aggiornato 9 ago 2026)
**001-076 applicate. ⚠️ La 077 è DA APPLICARE.**
**077** («Acconto di default nelle Impostazioni»: `workspaces.deposit_default_type` + `deposit_default_value`, con i due vincoli di coerenza). Validata su PG16. Senza, il campo in Impostazioni non salva e i preventivi nascono senza acconto preimpostato — nessun danno, solo la funzione che non parte. La 076 («la richiesta ricorda il preventivo») è stata applicata da Eli l'8 ago.
**~~Da applicare~~:** («la richiesta ricorda il preventivo che ne è nato»: `marketplace_requests.document_id` + GRANT per colonna, validata su PG16). Senza, il collegamento non si scrive e la richiesta resta com'era: nessun danno, solo la funzione che non parte.
