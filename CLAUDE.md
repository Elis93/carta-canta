...
**001-072 TUTTE APPLICATE**, con un'unica eccezione:
- ⚠️ **072 applicata solo a metà.** Eli ha eseguito la versione di ieri sera (funzione `security_meta_is_safe` + vincolo CHECK). Il blocco di **hardening aggiunto dopo la revisione** — REVOKE EXECUTE + `search_path` su `purge_old_security_events()` e `purge_old_stripe_events()` — **non è ancora stato eseguito**. Non urgente (senza, quelle due funzioni restano invocabili via RPC, ma fanno solo ciò che il cron fa comunque). Il SQL è in coda a `supabase/migrations/072_meta_eventi_solo_etichette.sql`, righe 61-71: è idempotente, si può lanciare quando capita.

Ultime applicate: 069 (chiude la lettura pubblica delle foto rimasta aperta dalla 041) · 070 (coordinate di pagamento modificabili solo dal server, anti-frode BEC) · 071 (`security_events`) · 072 (`meta` senza dati personali). Lo storico completo con le date sta in `STORICO_SESSIONI.md`.
