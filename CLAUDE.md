...
### ✅ 19 ago — Mockup Home «più belli» + Sopralluogo riorganizzato (titolo/cliente come Nuovo Preventivo)
Due richieste di Eli nello stesso messaggio.
- **[MOCKUP] Home «più bella»**: i mockup precedenti (compatti) erano «statici e tristi». Nuovo artifact `home-belle.html` con **due estetiche più calde**: **A «Sartoriale»** (bianchi crema, filetti oro, numeri in Fraunces, tasto Sollecita a pillola dorata) e **B «Vivace»** (sezioni con tinte tenui — rosso/ambra per l'urgenza, verde per gli incassi). Condiviso: **testata navy con bagliore d'oro radiale** + monogramma CC in filigrana + riga hero-stats (Da incassare · In attesa · In scadenza) + saluto «Ciao Elisa» in Fraunces. Tutte le sezioni di oggi restano (agenda, in scadenza, fattura elettronica, questo mese, attività recente). Verificato in Chromium a 336px (light+dark): zero overflow, dark leggibile. ⏸️ **In attesa che Eli scelga il tono** (A, B, o un mix es. testata di A + tinte di B) prima di costruire la Home vera.
- **[SOPRALLUOGO] Titolo + Cliente come «Nuovo Preventivo»** (`SopralluogoForm.tsx`): erano dentro un'unica sezione collassabile «Cliente e cantiere». Ora **titolo** (campo leggero «Metti il titolo», sempre visibile) e **Cliente** (card con etichetta, sempre visibile) sono card in cima, nello stile del preventivo. Restano collassabili solo Appunti, Foto e Appuntamento (i blocchi che «prendono spazio»). Nuova card **«Cantiere»** con l'indirizzo del cantiere + l'appuntamento annidato. **Tolta la parola «Facoltativo»** (placeholder indirizzo → «Indirizzo del cantiere»; riepilogo appuntamento «Nessuno · facoltativo» → «Nessuno»).
- **[SOPRALLUOGO, risposta alla domanda sull'indirizzo]** «se un cliente ha già un indirizzo si inserisce in automatico?». **No, e volutamente**: l'indirizzo del cliente (residenza/sede) NON è quello del cantiere — il lavoro può essere altrove (appartamento in affitto, seconda casa, altra unità). Quindi il campo «Indirizzo del cantiere» resta ed è la fonte di verità per la navigazione. Nuovo **`getClientAddressAction`** (in `lib/actions/clients.ts`): quando il campo cantiere è **vuoto** e il cliente ha un indirizzo in rubrica, compare un **tocco «Usa l'indirizzo di {cliente}: …»** che lo copia (poi modificabile). **Mai riempimento automatico**: così il campo mostra sempre esattamente ciò che verrà usato e non nascono due indirizzi divergenti senza accorgersene. FAQ agenda allineata alla card «Cantiere».
- tsc+build+**733** test verdi · scan spazi pulito.
