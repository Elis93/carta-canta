...
- **DESIGN DECISO da Eli (12 ago)**: restano **aperti i PRIMI 8 preventivi** (come nel flusso Free — i più vecchi per data); tutto il resto (preventivi #9+ e ogni funzione Pro) **si VEDE ma è BLOCCATO** e **non richiamabile** altrove. Esempio testuale di Eli: *«se è stato fatto un nuovo template con Pro, su Free l'utente vede quel template ma è bloccato e non utilizzabile nei documenti»*. I **dati Pro NON si cancellano**: restano salvati e visibili, solo bloccati → tornando Pro si sbloccano identici. Ovunque bloccato: messaggio **«Torna a Pro per sbloccare»** → `/abbonamento` (tono formale, § 11 ago).
- **⚠️ NUOVA DECISIONE Eli (12 ago) — il limite di 8 vale ANCHE per le FATTURE, e per TUTTI i Free (non solo i downgrade)**: *«farei un limite anche alle fatture, di 8; non solo per chi passa da Pro a Free ma anche per chi inizia Free — bloccherei le fatture a 8»*. Quindi il piano Free diventa **8 preventivi + 8 fatture** (dalla 9ª in poi: sola lettura, «Torna a Pro»). ⚠️ **Implicazioni da valutare prima di implementare (NON è più solo il downgrade)**: (a) cambia il piano Free per **tutti**, compresi gli utenti Free ATTUALI che potrebbero avere già >8 fatture → decidere se bloccarli o lasciarli (grandfathering); (b) oggi il contatore `sent_quota_used` è **solo preventivi** (commento in `lib/free-trial.ts`) → serve un contatore/logica per le fatture (probabile migration); (c) i **testi commerciali** («8 preventivi gratuiti» su landing/FAQ) andranno aggiornati a «8 preventivi e 8 fatture» → materia B.0 (claim, modifica del piano gratuito). Da confermare anche se 8+8 separati o 8 documenti totali (proposta: separati, come detto da Eli «anche alle fatture»).
- Un preventivo/fattura BLOCCATO resta **solo lettura** (si apre e si legge) ma non modificabile/inviabile — «visibile» = apribile in lettura, non solo una riga in lista.
- **STATO: solo valutazione** (Eli 12 ago: «per ora basta la valutazione») — nessun codice scritto; si implementa su sua richiesta, a fasi.
...
### Collaudo Eli 9 agosto 2026 (decisioni)
- ✅ **PRINCIPIO GENERALE — «rendiamo le cose più lineari e semplici possibile, riduciamo i fraintendimenti, e se crea rischio non facciamolo»** (Eli, 9 ago). Vale oltre il caso che l'ha originato: davanti a una funzione che *potrebbe* servire ma apre a un uso sbagliato, la risposta è **non farla** e spiegare bene l'alternativa, non farla con un avviso sopra. Un interruttore in più, un'eccezione in più, una regola con un «però» sono tutti modi di spostare il rischio addosso all'artigiano.
- ❌ **NIENTE nota di credito su fatture NON trasmesse allo SdI** (Eli, 9 ago, su mia raccomandazione). Domanda sua: *"ha senso che la nota di credito sia possibile farla anche se la fattura non è trasmessa a SdI ma solo inviata al cliente?"* — **no**. Una nota di credito non corregge un documento: rettifica un'**operazione che l'Agenzia ha già registrato**. Su una fattura mai trasmessa la TD04 chiederebbe indietro un'IVA mai dichiarata, e sarebbe **irreversibile** (una nota trasmessa si compensa solo con una nota di debito): un secondo buco al posto del primo. Per quel caso l'app ha già gli strumenti giusti e reversibili — **correggi e rimanda** (badge «Modificato»), **«Annulla fattura»**, **elimina** (cestino 15 giorni). ⚠️ Il caso in cui l'istinto opposto avrebbe ragione — l'artigiano che **emette altrove** (commercialista, Aruba) — resta scoperto per la stessa decisione dell'8 agosto sull'interruttore «l'ho già emessa altrove»: le due decisioni vanno lette insieme, e il vuoto si chiude da sé quando lo SdI sarà acceso dentro Carta Canta.
- 🔒 **Rete sotto la porta, non solo la porta**: la route di trasmissione **rifiuta** una TD04 la cui fattura d'origine non risulta trasmessa (e **fallisce chiusa** se lo stato non si riesce a leggere). Oggi il tasto «Crea nota di credito» compare già solo sulle fatture trasmesse: la guardia serve a fare in modo che non basti aprire una scorciatoia in UI per aggirare la decisione.

### Collaudo Eli 4-8 agosto 2026 (decisioni)
- ❌ **NIENTE interruttore «questa fattura l'ho già emessa altrove»** (decisione Eli, 8 ago, dopo la mia raccomandazione contraria). L'app blocca eliminazione e annullamento delle fatture **trasmesse da noi**; se l'artigiano emette la fattura con un altro programma non possiamo saperlo, e un interruttore da spuntare a mano NON è la risposta: protegge solo chi si ricorda di accenderlo (cioè non chi sbaglia), crea uno stato che non possiamo verificare né correggere, duplica un fatto che vive in un altro gestionale, e aggiunge una domanda fiscale in cantiere. La difesa resta: **avviso nel momento in cui sta per eliminare** + **cestino di 15 giorni** (l'errore è recuperabile) + la FAQ «Cosa posso eliminare e cosa devo tenere». Il problema sparisce da sé quando lo SdI sarà attivo dentro Carta Canta.
- ❌ **Il cestino resta com'è** (decisione Eli, 8 ago): *"va lasciata la possibilità di eliminare i documenti inviati al cliente ma non trasmessi"*. Avevo proposto di non far distruggere mai dal purge automatico le fatture già inviate al cliente — **scartata**. Il confine è uno solo e resta la **trasmissione allo SdI**: prima di quella l'artigiano è padrone dei suoi documenti (una fattura inviata per email e mai trasmessa non è emessa), dopo non si tocca più niente. Il cestino continua a fare quello che promette: 15 giorni, poi si svuota.
- ✅ **Se fiscalmente non si può, non si deve poter fare nemmeno qui** (Eli, 8 ago): principio confermato per le fatture trasmesse allo SdI — niente eliminazione, niente annullamento, niente ritorno in bozza. Già in vigore nel codice, verificato.

### Decisioni del 10 agosto 2026
- ✅ **Regime dei Minimi TOLTO** (*"assolutamente d'accordo a togliere se fatto male, crea confusione e ci mette a rischio"*): l'app lo trattava da ordinario (IVA addebitata, niente bollo né dicitura). Opzione via da onboarding e Impostazioni (resta visibile solo a chi l'avesse già); via anche dalla pagina di presentazione. Se arriverà un utente vero nei minimi, si riapre come progetto «gestirlo bene».
- ✅ **MULTI-NOTA di credito col TETTO** (*"procedi con la proposta e i tre punti come dici tu"*): più note parziali sulla stessa fattura; invariante «Σ note attive ≤ totale fattura». I tre punti: ① tetto BLOCCANTE alla trasmissione + avviso ambra sulla nota che supera il residuo (in bozza si lavora liberi); ② le note dopo la prima nascono con le voci ridotte in proporzione al residuo (arrotondate per difetto: nascono DENTRO il tetto); ③ a residuo zero il tasto è spento e spiegato. Migration 079 (via l'indice unico della 078).
...
- ✅ **Menu del form → "Note, foto e condizioni"** (2 ago, scelta via domanda): due blocchi «Note (e foto)» / «Condizioni», sottotitoli 13px, divisori tra le voci. Vale anche per FatturaForm ("Note e condizioni" in edit).
  - ⏩ **Superata il 7 set 2026** (mockup «Riordino di settembre», Eli: «1 ok»): il form ha QUATTRO sezioni con l'etichetta fuori dalla card — Intestazione (titolo + numero + cliente) · Voci · **Condizioni e allegati** (una riga a tendina per cosa: Note al cliente · Note interne · Foto · Acconto · Validità · Tempi di esecuzione · Pagamento · Template, col valore a destra) · Riepilogo (col Margine come ultima riga). La voce aperta chiede TRE campi (Quantità con l'unità dentro, Prezzo, IVA); sconto, calcolo quantità e bene significativo sotto «Sconto · …⌄». AI e catalogo nel menu di «Aggiungi voce». Vale anche per Nuova fattura.
...
- ⏳ **Il Fatturato della Home conta anche le fatture di acconto** (26 set 2026, decisione Eli: «Ok a contare anche gli acconti fatturati in home»). Si implementa con la **Fase 3**, insieme al saldo a conguaglio che uscirà al NETTO degli acconti: così acconto + saldo = totale del lavoro, senza doppio conteggio. Nel Bilancio invece l'incasso resta contato una volta sola sul preventivo (regola della Fase 2).
- ⏳ **Correzione di un acconto sbagliato: DOPO la Fase 3** (26 set 2026, decisione Eli). Il flusso completo (comando dal preventivo, TD02 già trasmessa, rimborso per lavoro saltato — domanda N22) resta in `PROGETTO_ACCONTI.md` §3 «da valutare».
...
- ✅ **Tipografia Home unificata** (4 ago sera, mockup approvato "mi piace, procedi"): da 9 dimensioni miste a 4 livelli fissi — titoletti 12, dettagli 13, riga principale 14 semibold, numeri 24. La classe `.cc-section-label` è l'unico livello titoletto (niente più override nei call site).
...
- ✅ **Foto in archivio privato con link a scadenza** (5 ago, dopo la domanda di Eli "ad oggi ancora nessuno usa la app, potrebbe essere il momento buono"): le foto dei lavori non hanno più un indirizzo permanente; ogni pagina che le mostra chiede un link che **scade dopo un'ora**. Fatto adesso proprio perché senza utenti reali il rischio di rompere qualcosa costava quasi nulla.
- ✅ **Card «Messaggi» dell'artigiano a tendina** (5 ago, richiesta di Eli): come sulla pagina del cliente. **Si apre da sola solo quando c'è da rispondere** (ultimo messaggio del cliente); se hai già risposto resta chiusa — è storia, non deve occupare spazio nel documento.
...
- ✅ **Termine dei lavori sul preventivo** (7 set, implementato — migration 088): «Tempi di esecuzione (giorni dalla conferma)» nel form, facoltativo e vuoto di default (clausola contrattuale: chi non lo compila non promette nulla); dicitura «Indicativamente entro N giorni dalla conferma del preventivo, salvo imprevisti o cause non dipendenti dall'impresa» in PDF (4 preset), pagina del cliente e riepilogo; dopo l'accettazione «Lavori entro il {data}», ripresa in testata nella scheda Lavoro (rossa se superata a lavoro non finito); default in Impostazioni › Generale. Solo preventivi: la fattura non lo eredita. Frase per l'avvocato in `COSE_DA_FARE_ELI.md §2`.
