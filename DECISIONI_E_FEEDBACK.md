...
- ✅ **Data contestuale nelle liste** (feat DATA_CONTESTUALE): nelle liste preventivi e fatture la data accanto a ogni documento è ora contestuale allo stato: bozza → "Modificato il…", inviato → "Inviato il…" o "Scade tra N g" (rosso se ≤7gg), accettato → "Accettato il…"/"Pagata il…", rifiutato → "Rifiutato il…"/"Annullata il…", scaduto → "Scaduto/Scaduta il…". Helper condiviso `lib/utils/document-date.ts` usato da entrambe le liste. Select Supabase aggiornate con `accepted_at, updated_at` (preventivi) e `sent_at, expires_at, accepted_at, updated_at` (fatture). Badge "Modificato" e logica di ordinamento invariati.
