...
- 🟡 **T-18** — **da riconfermare nel browser** (ultima sessione: FIX-POPUP-CLICK). Storia: FIX-16 ha portato la tendina su portale `document.body` per evitare il clipping dall'`overflow-y-auto` del `DialogContent`. Ma il `DismissableLayer` interno di Radix trattava i click sulla tendina (fuori dal DOM del dialog) come "pointerdown outside" → chiudeva il dialog prima che `onMouseDown` potesse selezionare il cliente. **Fix (FIX-POPUP-CLICK):** aggiunto `data-dropdown-portal` al `<ul>` della tendina; nel `DialogContent` aggiunto `onPointerDownOutside={(e) => { if (e.target.closest('[data-dropdown-portal]')) e.preventDefault() }}` — blocca il dismiss di Radix quando il click è sulla tendina, lasciando che `onMouseDown` del bottone lista completi la selezione. Stesso attributo aggiunto a `ClientAutocomplete.tsx` per coerenza futura. I tentativi precedenti (onBlur 300ms, Radix Popover+onInteractOutside, FIX-15 senza portale) erano stati segnati ✅ ma NON funzionavano nel test reale — questo NON va segnato ✅ finché Eli non lo conferma nel browser. Vedi CLAUDE.md sessioni FIX-16/FIX-17/FIX-18/FIX-19/FIX-20/FIX-POPUP-CLICK.
