...
- 🟡 **T-18** — **da riconfermare** (sessione FIX-15): suggerimenti cliente in `SendEmailDialog` (`ClientSearchInput`) e `ClientAutocomplete` riscritti SENZA Radix Popover — tendina `<ul>` assoluta nel wrapper `relative`, apertura via `isFocused` locale, selezione con `onMouseDown`+`preventDefault`, chiusura solo su selezione/Esc/blur fuori dal wrapper. I tentativi precedenti (onBlur 300ms, poi Radix Popover+onInteractOutside) erano stati segnati ✅ ma NON funzionavano nel test reale — questo NON va segnato ✅ finché Eli non lo conferma nel browser. Vedi CLAUDE.md sessione FIX-15.
