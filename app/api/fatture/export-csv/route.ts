...
export async function GET() {
  ...
  const bloccato = await guardExport({ userId: user.id, workspaceId: workspace.id, what: 'fatture' })
  ...
  const rows = ((fatture ?? []) as FtRow[]).map((ft) => {
    ...
    return [
      ...
      escapeCsv(
        isNc
          ? (ft.status === 'rejected' ? 'Nota di credito annullata' : 'Nota di credito')
          : ft.doc_type === 'nota_debito'
