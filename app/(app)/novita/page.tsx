...
const NOVITA: Array<{ data: string; titolo: string; punti: string[] }> = [
  {
    ...
    punti: [
      ...
      'Un preventivo da cui è nata una fattura — anche solo di acconto, anche non ancora trasmessa — non si può più eliminare: è l’origine di quella fattura, e l’acconto incassato vive lì, nelle Entrate del Bilancio. Il tasto Elimina resta spento e spiega il perché; per toglierlo dalla lista c’è «Archivia».',
    ],
  },
  {
    data: 'Fine settembre 2026',
  ...
  },
  {
    ...
    punti: [
      ...
      'La copia automatica si comanda cliente per cliente: nella scheda in rubrica c’è la spunta «Invia sempre la copia di cortesia». La togli per chi ha rinunciato alla copia o quando preferisci mandarla tu — per quel cliente non parte più nulla da solo, e sulla fattura resta il tasto «Invia».',
      'Se una fattura viene scartata dallo SdI dopo che il cliente ha già ricevuto la copia, l’app te lo dice — sulla fattura e nell’email di avviso — e, a correzione fatta ed esito positivo arrivato, ti ricorda di rimandargli la copia aggiornata.',
    ],
  },
  {
    ...
    punti: [
      ...
      'L’app è più comoda per chi legge o tocca con meno precisione: «Testo grande» ti viene proposto già alla registrazione (resta comunque in Altro), i tasti quadrati della Home sono più grandi e facili da centrare, e la busta del sollecito chiede conferma — col nome del cliente — prima di far partire l’email.',
    ],
  },
  {
    ...
    punti: [
      'I tre riquadri in cima alla Home dicono meglio cosa contano: solo il primo è un numero del mese («Fatturato di settembre»), e toccandolo si apre la lista delle fatture incassate in quel mese — con la pillola del filtro in cima, che togli con un tocco. «Preventivi in scadenza» e «Fatture da incassare» contano tutto ciò che è ancora aperto, di qualsiasi mese, e aprono le pagine con esattamente quei documenti.',
      'La lista dei sopralluoghi non usa più i cerchi con le sigle («GG», «S2»): ogni riga dice il cliente e l’indirizzo del cantiere e, sotto, lo stato in parole («Preventivo 012/2026 creato» in verde, «Da trasformare in preventivo» in grigio) con accanto l’appuntamento se deve ancora avvenire, altrimenti da quanto è fermo. Il filetto a sinistra è verde se il preventivo c’è già, oro se ci sono foto da lavorare, grigio se sono solo appunti.',
      'Nuovo preventivo e Nuova fattura riorganizzati in quattro sezioni, ognuna con la sua etichetta sopra la card: Intestazione (titolo, numero e cliente), Voci, Condizioni e allegati, Riepilogo. Ogni voce chiede tre campi — quantità con l’unità dentro il campo, prezzo, IVA — e il resto (sconto, calcolo della quantità, bene significativo) sta sotto «Sconto · …». Il tasto «Aggiungi voce» contiene tutte le strade: scrivi, dal catalogo o dai listini, dalle foto o da un PDF con l’AI. Le condizioni (note, foto, acconto, validità, tempi, pagamento, template) sono righe che si aprono una alla volta, col valore già leggibile a destra. Il margine è l’ultima riga del Riepilogo.',
      'Testi più ordinati in Home e nelle pagine dei documenti: dentro i riquadri bianchi ci sono ora quattro soli stili (etichetta di sezione, riga principale, testo secondario, cifra). Nelle tendine di preventivi, fatture e lavori il riepilogo a destra è sempre grigio e della stessa misura — anche «3 eventi» della Cronologia sta a destra come gli altri; restano colorati solo gli stati che chiedono attenzione (fattura elettronica scartata o vicina al termine, messaggi da rispondere). Il ⓘ della fattura elettronica ora sta dentro la card aperta.',
      ...
      'Sul preventivo non ci sono più «Segna accettato» e «Segna rifiutato» in pagina: l’esito lo dà il cliente dal link, con la prova. Per chi risponde al telefono restano nel menu ⋯ come «Segna accettato».',
      'Hai segnato accettato un preventivo per sbaglio? Nel menu ⋯ c’è «Segna come non accettato»: il preventivo torna in attesa (Inviato), non in bozza, e il link del cliente resta valido. Solo per le accettazioni fatte a mano: quelle firmate dal cliente non si annullano.',
      'Anche la scheda del Lavoro è riordinata: in testata (navy, come la Home) il titolo, il cliente e il cantiere, lo stato e i tre numeri — preventivato, speso, margine. Sotto, le ore in un riquadro solo, col timer dentro (Avvia · Ferma), le ore a mano e «Correggi il totale». A lavoro finito, subito sotto la testata, il tasto navy «Converti in fattura» (o il rapportino). Il resto è a tendine: Rapportino, Spese, Foto, Collegati, Richiama il cliente e «Stato e dettagli», senza più il tasto «Salva modifiche».',
      'Sempre sulla scheda del Lavoro, «Prossimo intervento» ha ora la sua card, separata da «Stato e dettagli»: chiusa dice la data e l’ora dell’appuntamento («gio 12 set · 09:00») o «Nessuno». Dentro «Stato e dettagli» restano lo stato, il titolo, il cliente, il cantiere e le note — con le etichette in tondo, più leggere.',
      'La scelta fra percentuale ed euro è ora un campo solo, uguale ovunque: il valore a sinistra e la pillola dell’unità dentro il campo — la tocchi e scegli % o €. Vale per lo sconto nel Riepilogo, per l’acconto del preventivo e per l’acconto proposto in Impostazioni › Generale (lo stesso schema dell’unità di misura nella quantità della voce).',
      'Piccole rifiniture: aprendo il menu del profilo la tendina delle notifiche si chiude da sola; attivando «Proponi più opzioni» la pagina scorre alla fascia «Stai compilando la proposta»; i riepiloghi delle righe chiuse sono più corti, così non si troncano nemmeno in «Testo grande».',
      ...
      'Sul preventivo puoi indicare i tempi di esecuzione in giorni dalla conferma (riga «Tempi di esecuzione», facoltativa, nella sezione Condizioni e allegati). Sul documento compare «indicativamente entro N giorni dalla conferma, salvo imprevisti» — la frase resta indicativa anche dopo l’accettazione: nessuna data calcolata al posto tuo. Un valore di partenza si imposta in Impostazioni › Generale.',
  ...
  },
  {
    ...
    punti: [
      'La pagina che il cliente apre dal link ora è una «ricevuta» elegante: il numero del documento grande al centro, la scadenza in evidenza, il totale in grande sotto un filetto scuro e, in fondo, il marchio Carta Canta. Stessi contenuti, molto più ordine.',
      ...
      'La fattura si crea anche dal Lavoro: sulla scheda di un lavoro finito, nella card «Documenti collegati», c’è «Converti in fattura». Riprende le voci del preventivo (il prezzo pattuito); ore e spese del lavoro restano il tuo margine, e gli extra concordati li aggiungi prima di inviarla.',
    ],
  },
  {
    data: 'Fine agosto 2026',
  ...
  },
  {
    ...
    punti: [
      ...
      'La proposta scelta si vede (e si può cambiare idea): dopo l’accettazione leggi ovunque quale proposta vale — riga di stato, banner, riepilogo, cronologia. Se serve, «Segna come non accettato» rimette il preventivo in attesa e fa tornare disponibili tutte le proposte.',
