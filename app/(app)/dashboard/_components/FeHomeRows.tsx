...
function FeRow({ href, dotColor, label, sub, subColor, count }: {
  ...
  return (
    ...
    >
      ...
      <span className="cc-t-main" style={{ flexShrink: 0 }}>{label}</span>
      <span className={subColor ? 'cc-t-sub-strong' : 'cc-t-sub'} style={{ flex: 1, minWidth: 0, color: subColor, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {sub}
      </span>
      <span className="cc-t-num" style={{ flexShrink: 0 }}>{count}</span>
